...
- **`credential-extraction/`** (subdir of `kohler-anthem`) — Frida + AVD tooling for extracting `client_id`, `apim_subscription_key`, `api_resource` from the Konnect APK. Contains `frida_bypass.js` with SSL-pinning + root + emulator + license bypass already implemented.
