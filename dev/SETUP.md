...
5. **Extract the APK** by running these commands:
   ...
   ```bash
   adb pull /data/app/com.kohler.hermoth-XXXX/base.apk dev/apk/base.apk
   ```
...
4. Save the file as `dev/apk/base.apk`

**Verify you have the APK:**
```bash
ls -la dev/apk/base.apk
```
...
1. Try manually searching the `dev/build/decompiled` folder
...
**Our Frida script (`dev/scripts/frida_bypass.js`) bypasses ALL FOUR layers** and also hooks SecurePreferences to capture the APIM key when the app stores it.
...
Saving APIM key to: dev/build/captured_apim_key.json
```
...
## Step 6: Install in Home Assistant
...
1. **Missing bypass messages** - The script didn't inject properly. Try:
   ```bash
   frida -U -f com.kohler.hermoth -l dev/scripts/frida_bypass.js --no-pause
   ```
...
```bash
grep -r "client_id" dev/build/decompiled/
grep -r "msal" dev/build/decompiled/
```
